...
public class SnapshotAdapter {
    ...
    public static List<HBaseProtos.SnapshotDescription> getSnapshotDescriptions(HBaseAdmin admin, String targetSnapshotName) throws IOException {
        // todo add test
        List<HBaseProtos.SnapshotDescription> result = new ArrayList<>();

        if (targetSnapshotName.endsWith(".*")) {
            String snapshotPrefix = targetSnapshotName.replaceAll(".*", "");
            for (HBaseProtos.SnapshotDescription snapshotDescription : admin.listSnapshots()) {
                if (snapshotDescription.getName().startsWith(snapshotPrefix))
                    result.add(snapshotDescription);
            }
        } else {
            for (HBaseProtos.SnapshotDescription snapshotDescription : admin.listSnapshots()) {
                if (snapshotDescription.getName().equals(targetSnapshotName)) {
                    result.add(snapshotDescription);
                    break;
                }
            }
